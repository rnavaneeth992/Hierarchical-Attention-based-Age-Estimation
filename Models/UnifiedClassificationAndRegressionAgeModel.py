...
class FeatureExtractionResnet34(nn.Module):
	def __init__(self):
		...
		pretrained_model_file = os.path.join(pretrained_model_path, "resnet_34.pt")
...
class UnifiedClassificationAndRegressionAgeModel(nn.Module):
	def __init__(self, num_classes, age_interval, min_age, max_age, device=torch.device("cuda:0")):
		super(UnifiedClassificationAndRegressionAgeModel, self).__init__()
